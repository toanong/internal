...
public class Record 
{
	 ...
	 * @param values
	 */
	public Record (String values[], boolean isEncrypted, boolean isLinked, String[] parSalts)
	{	
		...
		for (String key : writeOrder)
		{
			...
			if (getHashMap().containsKey(key))
			{
				...
				if ( s.equals(notInMapValue))
				{
					...
					encryptionFilter.addAsBigrams(getValue(key), null);
		...
		for (String key : writeOrder)
		{
			...
			if (getHashMap().containsKey(key))
			{
				...
				if (s.equals(notInMapValue))
				{
					...
					threadedEncrpytionFilters.get(threadNum).addAsBigrams(getValue(key), null);
	...
	{
		encryptionFilter = new BloomFilter<String>(bitSetSize, expectedNumberOElements, salts);
	}
	...
	public static void setThreadedEncryptionFilters(int numThreads)
	{
		...
		for (int i = 0; i < numThreads; i++)
			threadedEncrpytionFilters.add(new BloomFilter<String>(defaultBitSetSize, defaultExpectedNumberOElements, salts));
	}
