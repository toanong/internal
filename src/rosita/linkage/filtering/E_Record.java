...
public class E_Record 
{
	 ...
	 */
	public E_Record(String inputValues[], String inputTypes[], String[] parSalts)
	{
		if (!setupComplete)
	...
	public void encryptValues()
	{
		...
		for (int i = 0; i < values.length; i++)
		{
			...
			else
			{
				...
				encryptionFilter.addAsBigrams(s, salts);
		...
		for (int i = 0; i < values.length; i++)
		{
			...
			if(types[i].equals("yes")){values[i] = s;}
			 ...
			 else
				{
					...
					threadedEncryptionFilters.get(threadNum).addAsBigrams(values[i], salts);
	 ...
	 */
	public static void setup(MappingConfig mapConfig, int numFilters, String[] parSalts)
	{
	...
	private static void setupFilter(int numFilters)
	{
		...
		encryptionFilter = new BloomFilter<String>(defaultBitSetSize, defaultExpectedNumberOfElements, salts);
		threadedEncryptionFilters = new ArrayList<BloomFilter<String>>(numFilters);
		for (int i = 0; i < numFilters; i++)
			threadedEncryptionFilters.add(new BloomFilter<String>(defaultBitSetSize, defaultExpectedNumberOfElements, salts));
